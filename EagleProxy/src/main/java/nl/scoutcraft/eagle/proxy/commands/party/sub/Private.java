...
public class Private extends PartySubCommand {

    public Private(PartyManager partyManager) {
        super(partyManager, PartyMessages.INFO_PRIVATE, "", Perms.PARTY_PRIVATE, "private");
    }
    ...
    public void onCommand(Player player, String[] args) {
        ...
        if (!party.isPublicParty()) {
            PartyMessages.ALREADY_MODE.send(player, new Placeholder("%state%", "private"));
            return;
        }

        party.setPublicParty(false);
        PartyMessages.NOW_MODE.send(player, new Placeholder("%state%", "private"));
    }
}
