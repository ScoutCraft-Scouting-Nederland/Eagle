...
public class ScoutPlayerArgument extends Argument<ScoutPlayer> {

    public ScoutPlayerArgument(String name) {
        super(name);
    }

    @Nullable
    @Override
    public ScoutPlayer transform(CommandSource sender, String arg, CommandContext context) {
        return EagleProxy.getInstance().getPlayerManager().getScoutPlayer(CommandUtils.find(arg).orElse(null));
    }
